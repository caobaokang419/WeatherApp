 ...
 */
public class DbProviderClient implements IDbProviderClient {
    private static final int MAX_BATCH_APPLY_NUM = 100;

    private static DbProviderClient dbProviderManagerImpl;
    private Context context;

    private DbProviderClient() {
        context = WtApplication.getContext();
    }

    public synchronized static void registerInstance() {
        if (dbProviderManagerImpl == null) {
            dbProviderManagerImpl = new DbProviderClient();
        }

        Repository.Ext.setDbProviderManager(dbProviderManagerImpl);
    }
